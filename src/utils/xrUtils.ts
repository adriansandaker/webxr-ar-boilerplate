...
/* Returns true if navigator has xr with immersive-ar, otherwise false. */
export async function browserHasImmersiveArCompatibility(): Promise<boolean> {
    if (window.navigator.xr) {
        const isSupported: boolean = await navigator.xr.isSessionSupported('immersive-ar');
...
export function displayUnsupportedBrowserMessage(): void {
    ...
    middleMessage.innerText = 'Your browser does not support augmented reality with WebXR.';
    ...
    helpMessage.innerText = 'Try again using a recent version of Chrome on Android.';
